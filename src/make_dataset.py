...
#files = [f for f in files if "combined_features.csv" not in f]
...
def read_data_from_files(files):
    ...
    print(f"Accelerometer DataFrame shape: {acc_df.shape}")
    print(f"Gyroscope DataFrame shape: {gyr_df.shape}")

    acc_df.index = pd.to_datetime(acc_df["epoch (ms)"], unit="ms", errors="coerce")
    gyr_df.index = pd.to_datetime(gyr_df["epoch (ms)"], unit="ms", errors="coerce")

    acc_df.drop(["epoch (ms)", "time (01:00)", "elapsed (s)"], axis=1, inplace=True, errors="ignore")
    gyr_df.drop(["epoch (ms)", "time (01:00)", "elapsed (s)"], axis=1, inplace=True, errors="ignore")
